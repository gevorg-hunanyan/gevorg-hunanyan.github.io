...
Let \(F(n,d)\) denote the maximum number of floors we can handle with \(n\) eggs and at most \(d\) drops. We've already seen that \(F(2,6)=21\), for instance. The key observation here is that if we have \(n\) eggs and \(d\) drops left, and we drop an egg from floor \(x\) it will cost us one drop (so \(d\) becomes \(d-1\)), and the egg will either break or not. In case it breaks, we are left with \(n-1\) eggs and \(d-1\) drops to check the floors below \(x\), of which there are at most \(F(n-1,d-1)\). In case it doesn't break we are left with \(n\) eggs and \(d-1\) drops to check the floors above \(x\), of which there are at most \(F(n,d-1)\). Together with \(x\)-th floor these make up the whole \(N=F(n,d)\)-story building (see the picture below).
...
\[F(0,d)=0\;\text{(no eggs, nothing to test),}\]  
\[F(n,0)=0\;\text{(no drops, nothing to test).}\]
